...
ConsoleVariable  filesystem_log("filesystem", "log", "Log file to write.", "logfile.txt", CVAR_NONE );
ConsoleVariable  filesystem_path("filesystem", "path", "Current directory.", "", CVAR_NEEDSREFRESH );
ConsoleVariable  filesystem_logging("filesystem", "logging", "Is file logging enabled?", "0", CVAR_NEEDSREFRESH );
...
void ncFileSystem::Initialize( const char *defaultpath ) {
    ...
    if(strlen(defaultpath) > MAX_PATH) {
        _core.Error( ERC_FS, "Current application is placed far far away. Please make a path shorter.\n", MAX_PATH );
        return;
    }
    ...
    /* Notify */
    _core.Print( LOG_INFO, "Working directory: \"%s\"\n", defaultpath );
    ...
    if( filesystem_logging.GetInteger() ) {
        ...
        if( !( _gconsole.logFile = fopen( _stringhelper.STR("%s/%s", defaultpath, filesystem_log.GetString() ), "ab+" ) ) ) {
            _core.Print( LOG_ERROR, "Couldn't find or load the '%s' file for logging.\n");
    ...
    _commandManager.Add( "writecfg", lazyFileGenerateConfig );
}
...
void ncFileSystem::GenerateConfigurationFile( void ) {
    if ( _commandManager.ArgCount() < 1 ) {
        _core.Print( LOG_INFO, "usage: writecfg <raw>\n" );
        _core.Print( LOG_INFO, "raw: \"1\" - write default values\n" );
        _core.Print( LOG_INFO, "raw: \"0\" - write used values\n" );
        return;
    }

    if( atoi(_commandManager.Arguments(0)) < 0 )
        return;

    if( atoi(_commandManager.Arguments(0)) > 1 )
        return;
    ...
    // Create new file.
    if ( !( _cfg = OpenWrite( _stringhelper.STR("%s/config.cfg", filesystem_path.GetString()) ) ) ) {
#ifdef WARN_LEVEL_ERROR
        _core.Error( ERC_FS, "couldn't generate config file\n" );
#else
        _core.Print( LOG_ERROR, "Couldn't generate configuration file ( please check folder permissions ).\n" );
#endif
        return;
    }

    // *sign it*
    fprintf( _cfg, "// Generated by NanoCat %s\n", _version );
    fprintf( _cfg, "// for %s profile\n", namevar.GetString() );
    fprintf( _cfg, "// at %s\n", _date );
    fprintf( _cfg, "\n" );

    // Since I have write new console variable system, I need to
    // TODO this.

    _core.Print( LOG_INFO, "Done writing the configuration file. :)\n" );
    fclose( _cfg );
}
...
void ncFileSystem::ReadConfiguration( void ) {
    ...
    if( _commandManager.ArgCount() < 1 ) {
        ...
        _core.Print( LOG_INFO, "Usage: exec <configfile>\n" );
        return;
    }

    cfg = OpenRead( _stringhelper.STR("%s/%s.cfg", filesystem_path.GetString(), _commandManager.Arguments(0)) );
...
/*
    No WARN_LEVEL_ERROR here, 
    file writing is too important.
*/
...
void ncFileSystem::Read( FILE *f, void *data, long len ) {
    if ( fread( data, 1, len, f ) != (size_t)len )
        _core.Error( ERC_FS, "Filesystem failed to read file." );
}
