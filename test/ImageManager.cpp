...
/*
    Write image.
*/
bool ncImageLoader::CreateImage( int width, int height, byte *data, ncImageType type, const char *filename ) {
    ...
    switch( type ){
        ...
        case TGA_IMAGE_UNCOMPRESSED:
            ...
            break;
        default:
            _core.Error( ERC_ASSET, "ncImageLoader::CreateImage - Undefined image type given while loading %s.\n", filename );
            break;
...
/*
    Load image.
*/
bool ncImageLoader::Load( ncImageType type, const char *filename, ncImage *img ) {
    ...
    switch (type) {
        ...
        case TGA_IMAGE_UNCOMPRESSED:
            ...
            if (!imgfile) {
                _core.Print( LOG_ERROR, "ncImageLoader::Load - Couldn't find %s image.\n", filename );
                return false;
            }
            ...
            if (img->type != 2 && img->type != 3) {
                _core.Print( LOG_ERROR, "ncImageLoader::Load - %s.tga is corrupted\n", filename );
                fclose(imgfile);
                return false;
            }
        ...
        case BMP_IMAGE:
            ...
            if ( fread(header, 1, 54, imgfile) != 54 ) {
                _core.Print( LOG_ERROR, "ncImageLoader::Load - %s.bmp is corrupted\n", filename);
                return false;
            }

            if ( header[0] != 'B' || header[1] != 'M' ) {
                _core.Print( LOG_ERROR, "ncImageLoader::Load - Image %s is not a BMP.\n", filename);
                return false;
            }
        ...
        default:
            _core.Print( LOG_DEVELOPER, "ncImageLoader::Load - Undefined image type given while loading %s.\n", filename );
            return false;
            break;
    }
